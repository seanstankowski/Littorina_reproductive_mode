...
def phyml_wrapper(windowQueue, resultQueue, windType, model, opt, outgroup, phyml, minSites, minPerInd, minSNPs=None,
    ...
    #sys.stderr.write("******************* phyml_wrapper before while******")
    while True:
        #sys.stderr.write("+++++++++++++++++++++++++++++++phyml_wrapper++++++++++++++++++++++++")
        ...
        if Nsites >= minSites:
            ...
            sys.stderr.write("&&&&&&&&& stacked values: {} \n first {}, second {}".format(sitesPerInd, min(sitesPerInd), minPerInd))
            if min(sitesPerInd) >= minPerInd and (minSNPs is None or
                ...
                #sys.stderr.write("<------------------- before enough sites")
                if maxLDphase: aln = genomics.maxLDphase(aln)
                #sys.stderr.write("<---------------if enough sites get tree-------------")
...
#sys.stderr.write("----------------------------------- for x in range ____________________")
for x in range(threads):
    #sys.stderr.write("======================= THREAD {}".format(x))
